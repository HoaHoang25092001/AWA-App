...
const WriteIndex = () => {
  ...
  // const [inputThangSoDoc, setInputThangSoDoc] = useState("");
  // const [inputCanBoDocId, setInputCanBoDocId] = useState("");
  // const [inputTuyenDocId, setInputTuyenDocId] = useState("");
  // const [inputKhuVucId, setInputKhuVucId] = useState("");
  // const [inputKyGhiChiSoId, setInputKyGhiChiSoId] = useState("");
  ...
  const searchSoDoc = async () => {
    try {
      const nhaMayIdParams = Array.isArray(service)
        ...
        : `NhaMayIds=${service}`;
      const response = await apis.apiFilterSoDocChiSo(
        trangThaiSoDoc,
        nhaMayIdParams,
        inputTenSoDoc,
        currentPage,
  ...
  useEffect(() => {
    const nhaMayIdParams = Array.isArray(service)
      ? service.map((nhaMayId) => `nhaMayIds=${nhaMayId}`).join("&")
      : `nhaMayIds=${service}`;
    dispatch(fetchTrangThaiGhi(nhaMayIdParams)).then((result) => {
      if (result.payload) {
