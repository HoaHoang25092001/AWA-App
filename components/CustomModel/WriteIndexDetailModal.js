...
const WriteIndexDetailModal = ({
  ...
  const daGhiItem = trangThaiGhi.find((item) => item.keyId === "Đã ghi");
  ...
  const handleUpdateGhiChiSo = async () => {
    ...
    if (selectedItemId && imageSource) {
      setTrangThaiGhiId(daGhiItem.id);
      formData.append("RequestBody.TrangThaiGhiId", daGhiItem.id);
    } else {
    ...
    try {
      ...
      if (fecthUpdateGhiChiSo.fulfilled.match(result)) {
        ...
        if (payload.statusCode === 500) {
          ...
          setTrangThaiGhiSoDoc(!trangThaiGhiSoDoc)
        }
      } else {
  ...
  const getInvoiceChuaTTByMaKH = (maKhachHang) => {
    ...
    navigator.navigate("DebtInvoice", {
      itemId: itemId,
              ...
              <Modal.Body>
                ...
                {updateGhiChiSo?.hoaDonChuaThanhToan?.length > 0 ? (
                  <TouchableOpacity
                    onPress={() => {
                      setShowModal(false);
                      getInvoiceChuaTTByMaKH(updateGhiChiSo.maKhachHang);
                    }}
                    style={{
                      flexDirection: "row",
                      alignItems: "center",
