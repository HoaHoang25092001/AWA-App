...
const SamplePrint = ({ data, setStatusTT, statusTT, setStatusPrint, statusPrint }) => {
  ...
  const calculateInvoice = async () => {
    const payload = {
      ...
      thangTaoHoaDon: data.chiSoDongHo.thangTaoSoDoc,
      tuyenDocId: data.tenTuyen,
    };
    try {
      const response = await apis.apiTinhTienHoaDon(payload);
      console.log("Invoice", response);
      if (response?.statusCode == 201 || response?.statusCode == 200) {
        ToastAndroid.show("Thanh toán hóa đơn thành công !", ToastAndroid.LONG);
        setStatusPrint(!statusPrint)
        setStatusTT(!statusTT)
      } else {
        ToastAndroid.show("Thanh toán hóa đơn thất bại !", ToastAndroid.LONG);
      }
  ...
  <body>
    <div id="invoice-POS">
      ...
      : "BIÊN NHẬN CHƯA THANH TOÁN TIỀN NƯỚC"
    }
          </h5>
        </div><!-- End Info -->
      ...
      .join("")}
        ...
        data.tongTienTruocVat
            ...
            <tr class="tabletitle">
              ...
              <td></td>
              <td class="itemTax"><h4>${data.vat}</h4></td>
            </tr>
