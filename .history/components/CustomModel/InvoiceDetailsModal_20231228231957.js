...
const InvoiceDetailsModal = ({ visible, onClose, setStatusTT, paymentRecordID, statusTT }) => {
  ...
  // Tìm tháng muộn nhất
  const latestMonth = invoiceChuaTTByMaKH.reduce((latest, item) => {
    const currentDate = moment(item.chiSoDongHo.thangTaoSoDoc, 'MM/YYYY').toDate();
    if (!latest || currentDate.valueOf() > latest.valueOf()) {
      return currentDate;
    }
    return latest;
  }, null);
                    ...
                    <IntlProvider locale="vi-VN">
                      ...
                      {invoiceChuaTTByMaKH?.length > 0 ? (
                        invoiceChuaTTByMaKH?.map((data) => {
                          const currentMonth = moment(data.chiSoDongHo.thangTaoSoDoc, 'MM/YYYY').toDate();
                          console.log('Current Date:', currentMonth);
                          // Kiểm tra nếu tháng hiện tại lớn hơn tháng muộn nhất thì hiển thị button
                          const shouldDisplayButton = !latestMonth || currentMonth.valueOf() > latestMonth.valueOf();
