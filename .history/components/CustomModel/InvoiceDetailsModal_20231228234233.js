...
const InvoiceDetailsModal = ({ visible, onClose, setStatusTT, paymentRecordID, statusTT }) => {
  ...
  const [latestMonth, setLatestMonth] = useState('');
    ...
    // Tìm tháng muộn nhất
    const latestMonthTimestamp = Math.max(...invoiceChuaTTByMaKH?.map((item) => Date.parse(item.chiSoDongHo.thangTaoSoDoc)));
    const latesttMonth = invoiceChuaTTByMaKH?.find((item) => Date.parse(item.chiSoDongHo.thangTaoSoDoc) === latestMonthTimestamp).chiSoDongHo.thangTaoSoDoc;

    // Cập nhật state
    setLatestMonth(latesttMonth);
                        ...
                        invoiceChuaTTByMaKH?.map((data) => (
                          ...
                          >
                            ...
                            >
                              ...
                              {data.chiSoDongHo.thangTaoSoDoc === latestMonth && (
                                <View
                                  style={{
                                    flexDirection: "row",
                                    justifyContent: "center",
