...
const InvoiceDetailsModal = ({ visible, onClose, setStatusTT, paymentRecordID, statusTT }) => {
  ...
  const shouldDisplayButton = (dateString) => {
    const currentDate = moment(dateString, 'MM/YYYY');
    const latestMonth = moment(
      invoiceChuaTTByMaKH.reduce((latest, item) => (moment(item.chiSoDongHo.thangTaoSoDoc, 'MM/YYYY') > latest ? moment(item.chiSoDongHo.thangTaoSoDoc, 'MM/YYYY') : latest), moment())
    );

    return currentDate.isAfter(latestMonth, 'month');
  };
                        ...
                        invoiceChuaTTByMaKH?.map((data) => (
                          ...
                          >
                            ...
                            >
                              ...
                              {shouldDisplayButton(data.chiSoDongHo.thangTaoSoDoc) && (
                                <View
                                  style={{
                                    flexDirection: "row",
                                    justifyContent: "center",
