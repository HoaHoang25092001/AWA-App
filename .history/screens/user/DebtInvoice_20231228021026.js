...
import { getInvoiceById, getInvoiceChuaThanhToanByIdKH } from "../../store/HoaDon/asyncAction";

const DebtInvoice = ({ navigation, route }) => {
    const { itemId, trangThaiGhi, maKhachHang, statusTT } = route.params;
    const navigator = useNavigation();
    const dispatch = useDispatch()
    const { invoiceChuaTTByMaKH } = useSelector((state) => state.invoice);
    const [hoaDonChuaTT, setHoaDonChuaTT] = useState(invoiceChuaTTByMaKH);
    ...
    }, [dispatch, statusTT, navigation])

    useEffect(() => {
        setHoaDonChuaTT(invoiceChuaTTByMaKH)
    }, [invoiceChuaTTByMaKH])
    ...
    const getInvoiceInformationById = (id) => {
        ...
        navigator.navigate('CustomPrinter', { itemId: itemId, trangThaiGhi: trangThaiGhi, maKhachHang: maKhachHang })
    }
                            ...
                            <IntlProvider locale="vi-VN">
                                ...
                                <View style={{
                                    ...
                                    style={{
                                        ...
                                        fontSize: 16
                                    }}>Danh sách hóa đơn chưa thanh toán</Text>
                                </View>
                                {hoaDonChuaTT?.length > 0 ? hoaDonChuaTT?.map((data) => (
                                    <View
                                        key={data.id}
                                        style={{
                                            width: "100%",
                                    ...
                                    <View style={styles.emptyContainer}>
                                        ...
                                        <Text
                                            style={{ fontFamily: "Quicksand_700Bold" }}
                                        >
                                            Không còn danh sách hóa đơn chưa thanh toán
                                        </Text>
                                    </View>
                                )}
                            </IntlProvider>
