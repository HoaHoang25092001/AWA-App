...
function PaymentRecordScreen({ navigation }) {
    ...
    const onRefresh = React.useCallback(async () => {
        try {
            // Sử dụng async/await để đợi giá trị từ AsyncStorage
            const userId = await AsyncStorage.getItem('userId');
            console.log("userId", userId)
            // Gọi action với userId và các tham số khác
                ...
                <Modal.Content maxWidth="100%" maxH="600">
                    ...
                    <Modal.Footer>
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                console.log('idNguoiDung before dispatch:', idNguoiDung);
                                dispatch(getAllSoThanhToan({ currentPage, service, idNguoiDung }));
                                setTenSoDoc('');
                                setCreateDate(dayjs());
