...
function PaymentRecordScreen({ navigation }) {
    ...
    const [itemCount, setItemCount] = useState(soThanhToanTheoSoDoc?.items?.length);
    ...
    const onRefresh = React.useCallback(async () => {
        try {
            ...
            dispatch(getAllSoThanhToan({ currentPage, service, userId }));
        } catch (error) {
    ...
    useEffect(() => {
        fetchData();
    }, [isFocused, navigation, currentPage, service, dispatch]);
    ...
    useEffect(() => {
        if (soThanhToan !== soThanhToanTheoSoDoc) {
            setSoThanhToanTheoSoDoc(soThanhToan);
        }
    }, [soThanhToan, soThanhToanTheoSoDoc]);

    useEffect(() => {
        setItemCount(soThanhToanTheoSoDoc?.items?.length);
    }, [soThanhToanTheoSoDoc]);
    ...
    // };
    const PaymentRecordCard = React.memo(({ data }) => {
        const createdTimee = moment(data?.createdTime, 'MM/DD/YYYY').format('DD/MM/YYYY')
        return (
            <TouchableOpacity
    ...
    return (
        <SafeAreaView style={{ marginBottom: "35%" }}>
            ...
            <FlatList
                ...
                renderItem={({ item }) => <PaymentRecordCard data={item} />}
                refreshControl={
                    <RefreshControl
                        refreshing={refreshing && itemCount > 0}
                        onRefresh={onRefresh}
