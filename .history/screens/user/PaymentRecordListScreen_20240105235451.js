...
import {
  ...
  getInvoiceById,
  getInvoiceChuaThanhToanByIdKH,
} from "../../store/HoaDon/asyncAction";
...
import ModalScanQRCode from "../../components/CustomModel/ModalScanQRCode";

const { width, height } = Dimensions.get("window");

function PaymentRecordListScreen({ route, navigation }) {
  const { paymentRecordID, statusPrint } = route.params;
  ...
  const { soThanhToanBySoDoc, statusSTT } = useSelector(
    (state) => state.invoice
  );
  ...
  const [selectedIdKhachHang, setIdKhachHang] = useState();
  ...
  const onRefresh = React.useCallback(() => {
    dispatch(
      getAllSoThanhToanBySoDoc({ paymentRecordID, currentPage, service })
    );
  });

  // Store the previous page value for comparison
  const previousPage = useRef(currentPage).current;
  ...
  const searchHoaDon = async () => {
    try {
      const nhaMayIdParams = Array.isArray(service)
        ...
        : `nhaMayIds=${service}`;
      const response = await apis.apiFilterHoaDon(
        inputTenKH,
        trangThaiTT,
        inputSoTieuThu,
        inputSDT,
  ...
  const PaymentRecordListCard = ({ data }) => {
    const collectedTime = moment(data.ngayThu, "MM/DD/YYYY h:mm:ss A Z").format(
      "DD/MM/YYYY"
    );
    const paymentStatus =
      ...
      >
        ...
        <IntlProvider locale="vi-VN">
          ...
          >
            ...
            >
              {data.tongTienHoaDon.toLocaleString("en-US")} VNĐ
            </Text>
          </View>
          <View
            style={{
        ...
        >
          ...
          <Button
            ...
            onPress={() => {
              ...
              setIdKhachHang(data.idKhachHang);
            }}
          >
            <Ionicons color={"white"} name={"add-outline"} size={18} />
          </Button>
        </View>
        <TouchableOpacity
          onPress={() => {
  ...
  return (
    ...
    >
      <SafeAreaView>
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <Pagination
            ...
            pageSize={10}
            currentPage={hoaDonTheoSoDoc.pageNumber}
                ...
                <VStack space="md">
                  ...
                  <Box maxW="100%" alignItems="center">
                    <Input
                      ...
                      onChangeText={handleChangeTenKH}
                      value={inputTenKH}
                  ...
                  <Box maxW="100%" alignItems="center">
                    <Input
                      ...
                      size="md"
                      color="blue.500"
                      onChangeText={handleChangeTieuThu}
                      focusOutlineColor={"blue.200"}
                      placeholder="Nhập số tiêu thụ"
                      value={inputSoTieuThu}
                      style={{ fontFamily: "Quicksand_500Medium" }}
                      _light={{
                        bg: "light.50",
                        _hover: {
                  ...
                  <Box maxW="100%" alignItems="center">
                    <Input
                      ...
                      size="md"
                      color="blue.500"
                      onChangeText={handleChangeSoDienThoai}
                      focusOutlineColor={"blue.200"}
      ...
      <ModalScanQRCode
        selectedIdKhachHang={selectedIdKhachHang}
