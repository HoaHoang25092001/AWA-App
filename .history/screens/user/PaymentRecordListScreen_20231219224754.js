...
import { Center, Skeleton, Modal, Box, VStack, Input, Select, CheckIcon } from 'native-base'
...
function PaymentRecordListScreen({ route, navigation }) {
  ...
  const getInvoiceInformationById = (id) => {
    ...
    const searchHoaDon = async () => {
        try {
            const response = await apis.apiFilterHoaDon(inputTenKH, trangThaiTT, inputSoTieuThu, service, paymentRecordID, currentPage);
            console.log("Filtered hoa don", response.data);
            ...
            <SafeAreaView>
                <ScrollView showsVerticalScrollIndicator={false}>
                    ...
                    <View style={{
                        ...
                        {hoaDonTheoSoDoc.items.length > 0 ? hoaDonTheoSoDoc.items?.map(item => (
                        ...
                        <Box alignItems="center" w="100%">
                            <Box w="90%" maxW="400">
                                <VStack space="md">
                                    ...
                                    <Box maxW="100%" alignItems="center">
                                        <Input variant="outline" size="md" color="blue.500" focusOutlineColor={"blue.200"} onChangeText={handleChangeTenKH} value={inputTenKH} placeholder="Nhập tên khách hàng" _light={{
                                            bg: "coolGray.100",
                                            _hover: {
                                                bg: "muted.100"
                                            },
                                    ...
                                    <Box maxW="100%" alignItems="center">
                                        <Input keyboardType="numeric" variant="outline" size="md" color="blue.500" onChangeText={handleChangeTieuThu} focusOutlineColor={"blue.200"} placeholder="Nhập số tiêu thụ" value={inputSoTieuThu} _light={{
                                            bg: "coolGray.100",
                                            _hover: {
                                                bg: "muted.100"
                                            },
