...
export const apiFilterHoaDon = (tenKhachHang, trangThaiThanhToan, sanLuong, soDienThoai, nhaMayIds, soDocChiSoId, pageNumber) => axios({
    url: `hoa-don/filter-so-thanh-toan?TenKhachHang=${tenKhachHang}&trangThaiThanhToan=${trangThaiThanhToan}&SanLuong=${sanLuong}&SoDienThoai=${soDienThoai}&${nhaMayIds}&soDocChiSoId=${soDocChiSoId}&pageNumber=${pageNumber}&pageSize=10`,
    method: 'GET'
})

export const apiFilterSoDocChiSo = ( trangThaiSoDoc, nhaMayIds, tenSo, pageNumber, accessToken) => axios({
    url: `so-doc-chi-so/filter?TrangThaiSoDoc=${trangThaiSoDoc}&${nhaMayIds}&TenSo=${tenSo}&pageNumber=${pageNumber}&pageSize=10`,
    method: 'GET',
    headers: {
